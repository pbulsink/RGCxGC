setGeneric(name = "base_reference", def = function(joined_list, stat) {
...
setMethod(
  ...
  definition = function(joined_list, stat = "mean") {
    ...
    unfolded <- sapply(chroms, FUN = function(x) as.vector(t(x)))
    ...
    census_chrom <- matrix(mean_chrom, ncol = n_col, nrow = n_row, byrow = TRUE)
    census_chrom
  }
)

setGeneric(name = "method_reference", def = function(chroms, stat) {
