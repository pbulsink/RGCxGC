setGeneric(name = "base_joinChrom", def = function(x) {
  ...
  definition = function(x, y, groups, xy_names, ...) {
    ...
    if (length(others_chrom) > 0) {
      ...
      if (sum(!complex_condi) == length(complex_condi)) {
        ...
        others_joined <- lapply(
          rapply(others_joined, enquote, how = "unlist"),
          eval
        )
        ...
        others_joined_batch <- lapply(others_chrom_batch, base_joinChrom)
    ...
    if (!missing(groups)) {
      ...
      if (length(chrom_names) != nrow(groups)) {
        stop(
          "provided chromatograms and groups are
                     not the same length"
        )
      }
