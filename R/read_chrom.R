setGeneric(
  name = "base_GCxGC",
  def = function(Object, mod_time, sam_rate, per_eval, x_cut, y_cut, verbose) {
...
setMethod(
  ...
  definition = function(
    ...
    if (!homogeneous) {
      stop("Sampling rate is not homogeneuous")
    }

    # Calculate sampling rate
    if (missing(sam_rate)) {
      sam_rate <- scan_time[1:2]
    ...
    if (!is.null(x_cut) || !is.null(y_cut)) {
      if (!is.null(x_cut)) {
        ...
        if (length(cut_time) == 0) {
          stop(
            'Please provide a congruet time range to cut
                       chromatogram'
          )
        }
      ...
      if (!is.null(y_cut)) {
        ...
        if (length(cut_time) == 0) {
          stop(
            'Please provide a congruent time range to cut
                       chromatogram'
          )
        }
        cut_index <- raw_time %in% cut_time
...
#' @param sam_rate a integer, the sampling rate of the equipment in Hz.
