...
setClass(
  ...
  validity = function(object) {
    if (tools::file_ext(object@name) == "cdf") {
      ...
      RNetCDF::close.nc(val_chrom)
      if (inherits(val_chrom, "NetCDF")) {
        return(T)
      }
    } else if (tools::file_ext(object@name) == "csv") {
      # TODO: limit .csv files available
      return(T)
    } else {
      paste(object@name, "is not a valid NetCDF or csv file")
    }
  }
)
...
setClass(
  Class = "raw_GCxGC",
  slots = c(chromatogram = "matrix", time = "vector"),
  contains = "GCxGC"
)
...
setClass("batch_2DCOW", slots = c(Batch_2DCOW = "list"), contains = 'raw_GCxGC')
...
setClass(
  ...
  validity = function(object) {
    if (methods::is(objetc, "raw_GCxGC")) {
      ...
      print(
        "A chromatogram of class raw_GCxGC or 
...
setClass(
  "projected",
  slots = c(loadings = "list", time = "vector", mod_time = "numeric")
)
...
setClass(
  "MPCA",
  slots = c(scores = "matrix", summary = "list", groups = "data.frame"),
  contains = "projected"
)
