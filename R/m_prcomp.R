setGeneric(name = "base_MPCA", def = function(chrom, center, scale, npcs, ...) {
...
setMethod(
  ...
  definition = function(chrom, center, scale, npcs, ...) {
    ...
    if (length(col_removed) != 0) {
      ...
      warning(paste0(
        length(col_removed),
        "variables with 0 variance were
                             removed"
      ))
    }
    ...
    mpca <- list(scores = pca$x, loadings = loadings, summ = sum_pca)
    return(mpca)
  }
)
