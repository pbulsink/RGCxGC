setGeneric(name = "base_unfold", def = function(x) {
  standardGeneric("base_unfold")
})

setMethod(f = "base_unfold", signature = "list", definition = function(x) {
  if (any(names(x) %in% "")) {
    stop("A named chromatogram must be provided")
  }
  raw_signal <- sapply(x, function(x) as.vector(t(x)))
