setGeneric(name = "base_makelds", def = function(Object, D1_cols, D2_rows) {
  ...
  definition = function(Object, D1_cols, D2_rows) {
    m_loadings <- lapply(
      ...
      function(x, D1_cols, D2_rows) {
        m_lds <- matrix(x, nrow = D2_rows, ncol = D1_cols, byrow = TRUE)
        m_lds
      },
      D1_cols = D1_cols,
      D2_rows = D2_rows
...
make_loadings <- function(floadings, time, mod_time, acq_rate) {
  ...
  if (!(D1_cols * D2_rows == nrow(floadings))) {
    stop(
      "The length of the loadings does not match with the two-dimensional
