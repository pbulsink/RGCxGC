setGeneric(name = "minMax", def = function(data, tresh1 = 1, tresh2) {
...
setMethod(
  ...
  definition = function(sample_chrom, ref_chrom, segments, max_warp) {
    ...
    if (n_row_segs == 0) {
      ...
      n_row_segs <- 2
      print(
        "The segment length is forced to be smaller
    ...
    if (n_col_segs == 0) {
      ...
      n_col_segs <- 2
      print(
        "The segment lenght is forced to be smaller
    ...
    for (i in seq(1, len_d1 - 1)) {
      for (j in seq(ind_d1[i], ind_d1[i + 1] - 1)) {
        ...
        for (k in seq(1, len_d2 - 1)) {
          ...
          x_warp[j, tmp_idxr] <- tmp_xw[k] +
            seq(0, tmp_slen - 1) *
              (tmp_xw[k + 1] - tmp_xw[k] - 1) /
              (tmp_slen - 1)
        }
    ...
    for (i in seq(1, len_d2 - 1)) {
      ...
      x_warp[row_x, tmp_idxr] <- tmp_xw[i] +
        seq(0, tmp_slen - 1) *
          (tmp_xw[i + 1] - tmp_xw[i] - 1) /
          (tmp_slen - 1)
    }
    ...
    for (i in seq(1, len_d1 - 1)) {
      ...
      y_warp[tmp_idrx, col_x] <- tmp_xw[i] +
        seq(0, tmp_slen - 1) *
          (tmp_xw[i + 1] - tmp_xw[i] - 1) /
          (tmp_slen - 1)
    }
    ...
    for (i in seq(1, row_x)) {
      for (j in seq(1, col_x)) {
        ...
        x <- c - cl
        y <- r - rl
        aligned[i, j] <- sample_chrom[rl, cl] *
          (1 - y) *
          (1 - x) +
          sample_chrom[rl, cu] * x * (1 - y) +
          sample_chrom[ru, cl] *
            (1 - x) *
            y +
          sample_chrom[ru, cu] * x * y
      }
    }
    return(aligned)
  }
...
setMethod(
  ...
  definition = function(sample_chrom, ref_chrom, segments, max_warp) {
    ...
    al_chrom@chromatogram <- base_TwoDCOW(
      sample_chrom = sample_chrom@chromatogram,
