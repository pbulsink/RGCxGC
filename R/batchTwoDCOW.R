setGeneric(
  name = "base_batch_2DCOW",
  def = function(reference, sample_chroms, segments, max_warp, add_ref) {
...
setMethod(
  ...
  definition = function(reference, sample_chroms, segments, max_warp, add_ref) {
    multiple_alig <- lapply(
      sample_chroms,
      method_TwoDCOW,
      ref_chrom = reference,
      segments = segments,
    ...
    if (add_ref) {
      ...
      all_chromatograms <- multiple_alig
    }
...
setMethod(
  ...
  definition = function(
    ...
    chrom_2DCOW@Batch_2DCOW <- lapply(lst_aligned, function(x) x@chromatogram)
    return(chrom_2DCOW)
  }
)
...
batch_2DCOW <- function(
  ...
  batch_aligned <- method_batch_2DCOW(
    reference = reference,
    sample_chroms = sample_chroms,
    segments = segments,
    max_warp = max_warp,
