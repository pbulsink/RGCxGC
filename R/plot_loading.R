...
setGeneric(
  name = "plot_loading",
  def = function(Object, type = "n", pc = 1, thresh, ...) {
...
setMethod(
  f = "plot_loading",
  signature = "projected",
  definition = function(Object, type = "b", pc = 1, thresh, ...) {
    if (length(pc) > 1) {
    ...
    if (!(type %in% c("n", "p", "b"))) {
      stop(
        "Only negative (n), positive (p), or both (b) loadings
                   types are available"
      )
    }
    ...
    if (is(Object, "MPCA")) {
      ...
      if (length(!Object@loadings$var_col) > 1) {
        ...
        for (i in seq_along(filled_loading)) {
          ...
          mloading <- matrix(filled_loading, byrow = T, nrow = D1, ncol = D2)
          mloading <- t(mloading)
        }
      } else {
        mloading <- matrix(loading, nrow = D1, ncol = D2, byrow = T)
        mloading <- t(mloading)
      }
    } else {
      # Procedure only for PLSDA
    ...
    labx <- round(seq(Object@time[1], Object@time[2], length.out = 5), 2)
    laby <- round(
