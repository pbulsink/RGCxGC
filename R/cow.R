setGeneric('histc', def = function(values, edges) {
  standardGeneric('histc')
})
setMethod('histc', signature = 'numeric', definition = function(values, edges) {
  ledges <- length(edges)
...
setGeneric(name = "InterpCoeff", def = function(n, nprime, offs, rtn) {
  ...
  definition = function(Ta, X, seg, Slack, Options = c(0, 1, 0, 0, 0)) {
    ...
    if (pred_bound) {
      if (!all(seg[, 1] == 1 & seg[length(seg)] == length(Ta))) {
        stop(
          "End points must be equal to 1 and to
      ...
      if (Options[[3]]) {
        ...
        if (floor((dim_x[2] - 1) / (seg - 1)) != n_seg) {
          stop(
            "For non-fixed segment lengths the target and teh signal
      ...
      if (tmp > 0) {
        ...
        if (Options[[1]]) {
          print(paste0("segments: ", len_segs[2, 1] + 1, " Points x: ", n_seg))
        }
      }
    ...
    if (length(Slack) > 1) {
      if (length(Slack) <= n_seg) {
        stop(
          "The number of slack parameters is not equal to
    ...
    Bounds[1, ] <- apply(
      matrix(c(Bounds_a[1, ], Bounds_b[1, ]), nrow = 2, byrow = T),
      2,
      max
    )
    Bounds[2, ] <- apply(
      matrix(c(Bounds_a[2, ], Bounds_b[2, ]), nrow = 2, byrow = T),
      2,
      min
    )
    ...
    if (!pred_bound) {
      for (i in seq(1, n_seg - 1)) {
        ...
        Int_Coeff[[i]] <- InterpCoeff(
          n = len_segs[1, 1] + 1,
          nprime = nprm,
          offs = Slacks_vec,
          rtn = "coeff"
        )
        Int_Index[[i]] <- InterpCoeff(
          n = len_segs[1, 1] + 1,
          nprime = nprm,
          offs = Slacks_vec,
          rtn = "index"
      ...
      Int_Coeff[[n_seg]] <- InterpCoeff(
        n = len_segs[1, n_seg] + 1,
        nprime = nprm2,
        offs = Slacks_vec,
        rtn = "coeff"
      )
      Int_Index[[n_seg]] <- InterpCoeff(
        n = len_segs[1, n_seg] + 1,
        nprime = nprm2,
        offs = Slacks_vec,
        rtn = "index"
    ...
    for (i in seq(1, n_seg)) {
      ...
      for (j in seq(node_a, node_z)) {
        ...
        if (n_aa != 0) {
          ...
          if (Options[[2]]) {
            Cost_Fun <- matrix(Table[2, nodes_tablepointer], nrow = n_aa) +
              CCs_Node
          } else {
            Cost_Fun <- matrix(Table[2, nodes_tablepointer], nrow = n_aa) +
              CCs_Node^Options[2]
